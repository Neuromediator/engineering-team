All 51 unit tests pass with no backend code changes needed. Here is the complete test file content.

## Summary of results

I executed the unit test suite against the `account.py` backend module using Python's stdlib `unittest` framework:
...
The backend module in `account.py` already met all requirements and needed **no fixes**. Since no backend code changed, the Gradio app in `app.py` remains fully compatible (it uses the unchanged public API: `Account`, `get_share_price`, `SUPPORTED_SYMBOLS`).

## The complete test file (`test_account.py`)
