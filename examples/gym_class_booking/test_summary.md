All 21 unit tests pass, and the Gradio UI validation in `_validate.py` also passes. The backend code already handled the promoted-member overlap check correctly via `_promote_from_waitlist`, and the existing test suite includes the requested regression test (`test_waitlist_promotion_skips_overlap`). No backend changes were needed.

Here is the test file (`test_booking.py`):
...
- Ran `test_booking.py` → **21 tests, all passed (OK)**.
- Ran `_validate.py` → **"Validation OK: app.demo is a gr.Blocks instance"**.

The feedback's core requirement is covered by `test_waitlist_promotion_skips_overlap`: a member waitlisted for a full class who also books an overlapping class is **not** auto-promoted into the conflict when a spot opens. Correspondingly, `test_waitlist_promotion_non_overlapping` verifies that a waitlisted member with only non-overlapping bookings *is* promoted normally. Backend `_promote_from_waitlist` already runs the same `_check_overlap` guard as a normal booking, so no code changes were required, and the Gradio app continues to validate.