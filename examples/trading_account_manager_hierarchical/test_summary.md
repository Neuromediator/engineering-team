...
## `test_account_system.py` — Unit Tests
...
class TestBuy(unittest.TestCase):
    ...
    def test_buy_exceeds_cash_raises(self):
        before_balance = self.acct.get_balance()
        with self.assertRaises(InsufficientFundsError):
            self.acct.buy("GOOGL", 10)
        self.assertEqual(self.acct.get_balance(), before_balance)
        self.assertEqual(self.acct.get_holdings(), {})
        self.assertEqual(len(self.acct.get_transactions()), 1)
...
|---|---|---|
| `TestGetSharePrice` | 3 | Known prices (AAPL/TSLA/GOOGL), unknown symbol raises ValueError, case sensitivity |
| `TestAccountCreation` | 1 | Initial state: zero balance, empty holdings, empty transactions, zero P&L |
| `TestCashOperations` | 8 | Deposit (success, zero, negative), Withdraw (success, insufficient funds, zero, negative), transaction records |
| `TestBuy` | 6 | Successful buy, unknown symbol, zero/negative quantity, insufficient funds, multiple accumulations |
| `TestSell` | 6 | Successful sell, exceeding holdings, exact holdings removal, zero/negative quantity, unowned symbol |
| `TestQueries` | 6 | Holdings copy, balance, portfolio value, total deposits, profit/loss, transactions copy |
| `TestIntegration` | 2 | Full workflow (deposit → buy AAPL → buy TSLA → sell AAPL), sell-all edge case |